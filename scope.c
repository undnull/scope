...
static const char *frag2_src =
    ...
    "   vec4 ac = texture(afterburn, texcoord) * 0.99;                  \n"
    "   target = max(cc, ac);                                           \n"
    "}                                                                  \n";
...
static void on_framebuffer_size(GLFWwindow *window, int w, int h)
{
    ...
    for(i = 0; i < 3; i++) {
        ...
        glTextureParameterf(textures[i], GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTextureParameterf(textures[i], GL_TEXTURE_MIN_FILTER, GL_LINEAR);
...
{
    return cosf(20.0f * curtime + phase);
}

static float make_signal_Y(float curtime, float phase)
{
    return cosf(5.0f * curtime + phase * 0.5f);
}
...
int main(int argc, char **argv)
{
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        frametime = pasttime - curtime;
        pasttime = curtime;
        phase += frametime;
        swap += frametime;

        signal_value[0] = make_signal_X(curtime, phase);
        signal_value[1] = make_signal_Y(curtime, phase);
        push_signal(signal_value);
